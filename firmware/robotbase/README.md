...
- 默认开启：`RB_PWM_AUTO_CENTER = 1`
- 上电后在 `RB_PWM_AUTO_CENTER_MS` 时间窗内（默认 `2000ms`）对四通道做均值采样，自动更新运行时中心值。
- 采样数不足的通道（低于 `RB_PWM_AUTO_CENTER_MIN_SAMPLES`）会保留编译默认中心值。
- 自动对中未完成前，遥控运动逻辑保持停止，避免上电误动作。
